...
5. **If the session assigns me a designated working branch** (cloud sessions often assign a `claude/...` branch), I push my commits there as required, and then also land them on `main` immediately: `git fetch origin main && git rebase origin/main && git push origin HEAD:main`. The rebase rewrites my commits, so if the designated branch was already pushed I re-push it with `git push --force-with-lease origin HEAD:<designated-branch>` (a plain push is rejected as non-fast-forward). Force-with-lease is only ever allowed on a `claude/*` scratch branch owned by this session; I never force-push `main`, and never force-push any other branch. This file is [Your Name]'s standing permission for me to push without stopping to ask; it is not a way around GitHub's own controls, and it cannot grant credentials or token scopes I do not have.

   **Work only counts as saved once it is on the remote.** The autosync workflow (`.github/workflows/vault-autosync.yml`) can only fold in branches that actually reached GitHub, so it is not a safety net for a push that was *rejected*: those commits exist nowhere but this temporary workspace, and they are destroyed when the session ends. If a push is rejected (branch protection, a missing token scope, expired credentials), I never try to engineer my way around the rejection, and I never assume autosync will cover for me. I try the remaining authorized paths in order -- `main` directly, then the designated branch, then a fresh `claude/*` branch -- and if every one of them is rejected I tell [Your Name] plainly, before the session ends, that the work is unsynced and what is in it. Per rule 7, silent data loss is the one unforgivable failure.
6. **Resolve every merge conflict myself, immediately, without asking.** Vault conflict rules:
   ...
   - `.claude/settings.json`, `.mcp.json`, and other config: merge keys from both sides. If the same key genuinely differs, "newer" means the side whose last commit touching that file has the later committer date (`git log -1 --format=%cI <rev> -- <file>`), and that side's value wins. Never eyeball which value looks newer.
   - If a Markdown file is truly ambiguous, keep both versions with a short `<!-- merge: kept both, review -->` comment and move on.
   - **The keep-both fallback is for Markdown only.** A JSON or YAML file must always come out syntactically valid: merge the keys as above. Never write an HTML comment, a duplicated key, or both variants into a structured file -- that produces a config that will not parse. And a file that parses is not automatically correct: a wrong value silently changes behavior. So if the two sides cannot be ordered by the rule above (identical timestamps, or the file was rewritten wholesale on both sides), I do not guess: I keep `main`'s existing value so the file stays valid and behavior is unchanged, and I flag the specific conflicting key to [Your Name] under rule 7.
   - Never leave conflict markers in a file, never abort a merge, never ask [Your Name] to resolve one.
