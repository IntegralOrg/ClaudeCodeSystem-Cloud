...
5. **If the session assigns me a designated working branch** (cloud sessions often assign a `claude/...` branch), I push my commits there as required, and then also land them on `main` immediately: `git fetch origin main && git rebase origin/main && git push origin HEAD:main`. The rebase rewrites my commits, so if the designated branch was already pushed I re-push it with `git push --force-with-lease origin HEAD:<designated-branch>` (a plain push is rejected as non-fast-forward). Force-with-lease is only ever allowed when the designated branch matches `claude/*`, which is a scratch branch owned by this session. Never force-push `main`, and never force-push any other branch: if the session hands me some other designated branch, I push it normally, and if that push is rejected I leave it alone and let autosync converge `main` instead. This file is [Your Name]'s standing permission for me to push without stopping to ask; it is not a way around GitHub's own controls, and it cannot grant credentials or token scopes I do not have. If the remote rejects a push (branch protection, a missing token scope, expired credentials), I never try to engineer my way around the rejection: I push the designated branch and move on -- the vault's autosync workflow (`.github/workflows/vault-autosync.yml`) folds side branches into `main` automatically -- and if pushes keep getting rejected I say so plainly under rule 7.
6. **Resolve every merge conflict myself, immediately, without asking.** Vault conflict rules:
   ...
   - **The keep-both fallback is for Markdown only.** A JSON or YAML file must always come out syntactically valid: merge the keys, and if the same key genuinely conflicts, keep the newer value. Never write an HTML comment, a duplicated key, or both variants into a structured file -- that produces a config that will not parse. If a structured file has no safe deterministic resolution, keep the newer side intact, leave the file valid, and flag it to [Your Name] under rule 7 rather than guessing.
