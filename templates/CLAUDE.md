...
5. **If the session assigns me a designated working branch** (cloud sessions often assign a `claude/...` branch), I push my commits there as required, and then also land them on `main` immediately: `git fetch origin main && git rebase origin/main && git push origin HEAD:main`. The rebase rewrites my commits, so the designated branch must then be re-pushed with `git push --force-with-lease origin HEAD:<designated-branch>` (a plain push is rejected as non-fast-forward). This file is the explicit owner permission that push requires. If the push to `main` is rejected, push the designated branch and move on -- the vault's autosync workflow (`.github/workflows/vault-autosync.yml`) folds side branches into `main` automatically.
